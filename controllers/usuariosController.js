...
exports.cadastraUsuarios = (req, res, next) => { 
    ...
    mysql.getConnection((error, conn) => { 
        ...
        conn.query('SELECT * FROM usuarios WHERE email = ?',[req.body.email], (error, results) => {
            if(error) {return res.status(500).send({ error: error})}
            if(results.length > 0){
                return res.status(401).send({mensagem: 'Usuario já cadastrado!!.'})
            }else{
...
exports.loginUsuarios = (req, res, next) => { 
    ...
    mysql.getConnection((error, conn) => { 
        ...
        conn.query(query,[req.body.email], (error, results) => {
            ...
            if(error) {return res.status(500).send({ error: error})}
            if(results.length < 1){
                return res.status(401).send({mensagem: 'Falha da autenticação!!'})
            }
